...
export const Ed25519Route:FunctionComponent = function Ed25519Route () {
    ...
    async function handleVerify (ev:Event) {
        ...
        try {
            ...
            // For Ed25519 verification, we need the signature in base64url format
            ...
            if (verPublicKey.value.startsWith('did:key:')) {
                ...
                // Assume it's an encoded public key, convert to DID format
                const pubKeyBytes = decodeFromEncoding(verPublicKey.value, verEncoding.value)
                didKey = `did:key:z${bytesToBase58(pubKeyBytes)}`
            }

            const isValid = await ed25519Verify(msgStr, sigBase64url, didKey as `did:key:z${string}`)
    ...
    return html`<div class="route ed25519-route">
        <h1>Ed25519 Signature</h1>
